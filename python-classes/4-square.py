...
class Square:
    ...
    def size(self, value):
        ...
        if not isinstance(value, int):
            raise TypeError("size must be an integer")  # Raise an error if the value is not an integer
        if value < 0:
            raise ValueError("size must be >= 0")  # Raise an error if the value is less than 0
    ...
    def area(self):
        ...
        """
        return self.__size ** 2  # Calculate the area of the square by squaring the size
